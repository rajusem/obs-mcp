...
func NewAlertmanagerClient(apiConfig api.Config) (*RealLoader, error) {
	...
	basePath := client.DefaultTransportConfig().BasePath

	// Use the configured RoundTripper (carries TLS and auth settings),
	// falling back to http.DefaultTransport for backward compatibility.
	...
	transport := httptransport.NewWithClient(host, basePath, []string{scheme}, httpClient)
